...
if __name__ == "__main__":
    ...
    # Import Data
    plot_df = pd.read_csv(f"{data_folder}/figS1_data.csv", parse_dates=["Date"])
    state_pop = pd.read_csv(f"{data_folder}/figS1_pop.csv")
    ...
    # Save the figure
    fig1.write_image(f"{image_folder}/vaccination_rate_completeness.png", scale=4)

    print(
        f"--- Finished exporting figure  S1, took {time.time() - start_time:,.2f} seconds ---"
    )
