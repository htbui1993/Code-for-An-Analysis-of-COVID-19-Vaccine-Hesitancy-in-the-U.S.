...
if __name__ == "__main__":
    ...
    # Read in the data
    plot_df = pd.read_csv(f"{data_folder}/fig3_data.csv")
    ...
    fig = go.Figure(
        layout=dict(
            ...
            font=dict(family='CMU Serif', size=11, color="black"),
        )
    ).set_subplots(
        rows=2,
        cols=4,
    ...
    fig.update_layout(
        ...
        yaxis5=dict(title=r"$\text{VH}^b \text{ (Week 23)}$", tickfont_size=10),
    ...
    # Export the figure
    fig.write_image(f"{image_folder}/hesitant_state_v2.png", scale=4)
    print(f"--- Finished exporting figure  3, took {time.time() - start_time:,.2f} seconds ---")